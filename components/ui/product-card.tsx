...
export const ProductCard = ({ data, ...rest }: Props) => {
	return (
		<Container testID='product-card-container'>
			<Content {...rest} testID='product-card-content'>
				<Image source={{ uri: data.photo_url }} testID='product-card-image' />
				<Details testID='product-card-details'>
					<Identification testID='product-card-identification'>
						<Name testID='product-card-name'>{data.name}</Name>
						<Feather
							name='chevron-right'
							size={18}
							color={extendedTheme.colors.$shape}
							testID='product-card-chevron'
						/>
					</Identification>
					<Description testID='product-card-description'>
						{data.description}
					</Description>
				</Details>
			</Content>
			<Line testID='product-card-line' />
		</Container>
	)
}
